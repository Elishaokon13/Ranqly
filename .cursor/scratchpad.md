...
- **Vercel + responsive (Mar 2026):** `npm run build` succeeds locally (matches Vercel). Added root `vercel.json` (`framework: nextjs`, `installCommand` / `buildCommand`), `package.json` `engines.node >=20.9.0`. Layout: `viewportFit: "cover"`, main uses `.min-h-main` (100vh + 100dvh fallback). Globals: safe-area horizontal padding on `body`, tap highlight off, `.min-h-main`. Explore: mobile filter sheet respects safe-area; search/sort row stacks with full-width sort on small screens; filter button min height 44px; sort select `sm:min-w-48`. Footer: single-column grid on narrow phones, legal links wrap with touch-friendly min height. Settings tabs: `aria-label` + `aria-hidden` on icons for icon-only mobile. **Note for deploy:** Vercel runs Next only; set `NEXT_PUBLIC_*` (e.g. API base URL) in the Vercel project. If the Express API is not deployed as a separate service or Route Handlers, client calls must point at a reachable API URL.
- **Settings / DB sync (Mar 2026):** Audited `/settings` tabs (Profile, Notifications, Privacy, Security). Backend `PATCH /api/me` merges `preferences` JSON in DB (`mergePreferences` in `backend/src/routes/me.ts`). Fixed build: `SettingsPrefsPanels` was missing `lucide-react` imports (`ShieldCheck`, `Laptop`, `AlertTriangle`, `Download`, `Trash2`). Added `ME_UPDATED_EVENT` in `src/lib/api.ts` after successful `PATCH /api/me` and avatar upload so all panels refetch `GET /api/auth/me`. Prefs panels now use default preferences when `NEXT_PUBLIC_USE_MOCK_API=true` (no infinite spinner). Save handlers apply PATCH response to local state to match server.
...
## Lessons
- Vercel: default Next.js project uses `npm run build`; pin Node via `engines` for consistent CI. `viewportFit: cover` + `env(safe-area-inset-*)` avoids content under notches/home indicators on iOS.
- Settings prefs panels: always import icons used in JSX; run `npm run build` to catch missing symbols. When `isApiConfigured()` is false, initialize prefs state from `normalizeUserPreferences(null)` so UI does not spin forever.
